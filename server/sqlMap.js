...
var sqlMap = {
  ...
  user: {
  	...
  	selectID: 'select uid,uname,sex,address from user where uid = ? or uname = ?',
  ...
  manger: {
    ...
    selectMan: 'select mid, mname, phone, sex, power from manger where mid = ? or mname = ?',
    update: 'update manger set mid = ?, mname = ?, sex = ?, phone = ?, power = ? where mid = ?',
  },
  //商品
	product: {
    ...
    searchpro: 'select * from product where pid = ? or pname = ? order by CONVERT(pid,SIGNED)', 
	},
	//用户购物车列表
	shopCar: {
	...
	//菜单列表
	menu: {
    add: 'insert into menu(cid, cname, cimg, ctype, cauthor, cinfo, collect, examine) values (?, ?, ?, ?, ?, ?, ?, ?)',
    delete: 'delete from menu where cid = ?',
    search: 'select * from menu where examine = "审核通过"',
    searchAll: 'select * from menu',
    searchByCollect: 'select * from menu where examine = "审核通过" order by collect desc',
    searchType: 'select distinct ctype from menu',
    searchCid: 'select * from menu where cid = ?',
    searchCid1: 'select * from menu where cid = ? and examine = "审核通过"',
    ...
    searchMenu1: 'select * from menu where cid = ? or cname = ? or cauthor = ?',
    ...
    update: 'update menu set cname = ?,cauthor = ?,cimg = ?,cinfo = ?,collect = ?,ctype = ?,examine = ? where cid = ?'
	},
	//用户地址列表
	address: {
    ...
    searchThings: 'select cid from material where things = ?',
    update: 'update material set things = ?,weight = ? where id = ?'
  },
	step: {
    ...
    update: 'update step set stepInfo = ?,stepImg = ? where id = ?'
	},
	userOrder: {
	...
	//收藏表
	collection: {
		add: 'insert into collection(uid,cid) values (?, ?)',
		searchUid: 'select * from collection where uid = ? and cid = ?',
		delete: 'delete from collection where uid = ? and cid = ?',
		collectCount: 'select count(*) as menus from collection where uid = ?',
		search: 'select * from collection where uid = ?',
	}
}
