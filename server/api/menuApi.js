...
router.post('/searchByCollect', (req, res) => {
	var sql = $sql.menu.searchByCollect;
	conn.query(sql, function (err, result) {
	    if (err) {
	      console.log(err)
	    }
...
//通过菜谱名或用料查找
router.post('/searchByNameMat', (req, res) => {
	var sqlId = $sql.menu.searchByNameMat;
	var params = req.body;
	conn.query(sqlId, ["%"+params.cname+"%", "%"+params.things+"%"], function (err, result) {
	    if (err) {
	      console.log(err)
	    }
	    if(result[0]===undefined) {
...
router.post('/searchCid1', (req, res) => {
	var sql = $sql.menu.searchCid1;
	var params = req.body;
	conn.query(sql, params.cid, function (err, result) {
	    if (err) {
	      console.log(err)
...
router.post('/update', (req, res) => {
	var sql = $sql.menu.update;
	var params = req.body;
	console.log(params);
	conn.query(sql, [params.cname,params.cauthor,params.cimg,params.cinfo,params.collect,params.ctype,params.examine,params.cid], function (err, result) {
		if (err) {
		  console.log(err);
		}
		if (result) {
...
router.post('/add', (req, res) => {
  var sql = $sql.menu.add;
  var params = req.body;
  conn.query(sql,[params.cid,params.cname, params.cimg, params.ctype, params.cauthor, params.cinfo, params.collect, params.examine], function(err, result) {
    if(err) {
      console.log(err);
    }
    if(result) {
